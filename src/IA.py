...
dataset.addSample([1,1,1],[3])
dataset.addSample([1,0,1],[2])
dataset.addSample([0,1,0],[1])
dataset.addSample([0,0,1],[1])
dataset.addSample([1,0,1],[2])
dataset.addSample([0,1,1],[2])
...
É passada à rede, o conjunto de dados (dataset), "learningrate" é a taxa de aprendizado,
"momentum" tem por objetivo aumentar a velocidade de treinamento da rede neural e
diminuir o perigo da instabilidade.
'''
trainer = BackpropTrainer(network, dataset, learningrate=0.1, momentum=0.99)
...
test_data.addSample([1,1,0],[2])
test_data.addSample([1,0,0],[1])
test_data.addSample([0,1,1],[2])
test_data.addSample([1,1,1],[3])
